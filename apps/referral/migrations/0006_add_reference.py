"""Give every referral a case number: REF-2026-0001.
...
sequence reads the way anyone would expect — the first referral of 2026
is REF-2026-0001. The shared counter in
...
def backfill(apps, schema_editor):
    ...
    Model = apps.get_model("referral", "referral")
    ReferenceSequence = apps.get_model("reference_data", "ReferenceSequence")

    counters: dict[int, int] = {}
    for row in Model.objects.order_by("sent_at", "id").iterator():
        raised = getattr(row, "sent_at", None) or timezone.now()
        year = timezone.localtime(raised).year
        counters[year] = counters.get(year, 0) + 1
        row.reference = f"REF-{year:04d}-{counters[year]:04d}"
        row.save(update_fields=["reference"])

    for year, last in counters.items():
        seq, created = ReferenceSequence.objects.get_or_create(
            prefix="REF", year=year, defaults={"last_number": last},
        )
...
class Migration(migrations.Migration):
    dependencies = [
        ('referral', '0005_programmeenrolment_unique_household'),
